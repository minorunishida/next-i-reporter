...
Excelファイルのセル構造情報 (JSON) を受け取り、各セルが帳票上のどのような入力項目（クラスタ）に対応するかを推測してください。

## クラスタ型の判断基準

- FixedText (20): 値が入っていてユーザーが変更しないラベル・タイトル・見出し。
...
1. 空白セルで、近くにラベルがあれば入力クラスタ（KeyboardText, InputNumeric, Date等）
2. 値が入っているセルはほぼ FixedText（ラベル）か Calculate（数式あり）
...
- 0.95: 明らかにラベル (FixedText) または数式 (Calculate)
...
- FixedText: "Font=MS Gothic;FontSize=11;Weight=Normal;Color=0,0,0;Align=Left"

## 重要: 漏れなく検出すること

この帳票がどんな種類（点検シート、温度管理表、請求書、作業日報、チェックリスト等）であっても、すべての意味のあるセルをクラスタとして出力してください。

1. **ラベル・見出し** (FixedText): テキストが入っている全てのラベルセル
2. **入力欄**: 空白セルでも、隣にラベルがあれば入力クラスタとして検出。文脈から型を推測:
   - テキスト系 → KeyboardText
   - 数値系 → InputNumeric
   - 日付系 → Date
   - 時刻系 → Time
...
4. **テーブル構造**: ヘッダー行 (FixedText) と入力行の各セルを個別に検出
5. **大きな空白結合セル**: 文脈から推測 (備考→KeyboardText、写真→Image、署名→Handwriting)

セルに値がなくても「タブレットで入力されるべき欄」ならクラスタとして検出すること。
帳票の規模に応じて適切な数のクラスタを検出してください（小規模: 10-20件、中規模: 20-50件、大規模: 50件以上）。
装飾だけの空白セル（罫線のみで意味のない空白）はスキップしてOKです。`;

/**
 * FormStructure を AI に送り、クラスタ定義を推測させる
 */
export async function analyzeForm(
  ...
  const allClusters = applySmartDefaults(sheetResults.flat());
...
async function analyzeSheet(
          ...
          properties: {
            clusters: {
              ...
              items: {
                type: "object",
                properties: {
                  name: { type: "string", description: "クラスタ名 (日本語)" },
                  typeName: {
                    type: "string",
                    enum: [
                      "FixedText",
                  ...
                  type: {
                    type: "number",
                    description:
                      "クラスタ型の数値 (FixedText=20, KeyboardText=30, Date=40, Time=50, InputNumeric=65, Calculate=67, Select=70, Check=90, Image=100, Handwriting=119)",
                  },
  ...
  return parsed.clusters.map((c, i) => ({
    id: `${sheet.index}-${i}`,
    name: String(c.name ?? ""),
    type: Number(c.type ?? 20),
    typeName: String(c.typeName ?? "FixedText") as ClusterDefinition["typeName"],
    sheetNo: sheet.index,
